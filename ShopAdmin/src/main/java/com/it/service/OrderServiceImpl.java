...
public class OrderServiceImpl implements OrderService {
	...
	public OrdermainVO orderproc(CartmainVO cartmain) {	// om_code 반환(주문리스트에서 사용)
		// 주문 main insert(신규생성)
		// cm_code를 사용하여 장바구니 main과 sub 조회
		// om_code 조회
		// 주문 sub에 insert
		...
		for (CartsubVO item : cartsub) {	// 장바구니에 있는 모든 상품들을 insert
			...
			cartmapper.deletesuball(cartmain);	// 장바구니 상세 내용 삭제(cm_code 활용)
			cartmapper.deletemain(cartmain);	// 장바구니 삭제(cm_code 활용)
	...
	public List<OrderlistDTO> getListOrderList(@Param("pageNum") int pageNum, @Param("pageAmount") int pageAmount, @Param("a_id") String a_id) {
		return ordermapper.getListOrderList(pageNum, pageAmount, a_id);
	}
